...
async function registerRoute(req, res) {
    try {
        ...
        if (result.upsertedCount) {
            logger.info(`create new user finished successfully: email: ${req.body.email}`, req);
            res.status(201).json({error: false, msg: 'User create successfully, Please log in'});
        } else if (result.matchedCount) {
            logger.warn(`create new user failed user already exist: email: ${req.body.email}`, req);
            res.status(400).json({error: true, msg: 'User already exist'});
        } else {
            throw new Error('failed to register a user');
        }
    } catch (e) {
        logger.error(`Error during registerRoute route: ${e}`, req);
